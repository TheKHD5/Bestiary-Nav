...
- **Levelling (Experimental):** target enemies +1 to +10 levels above BST. Expand a zone to choose patrol locations and target or ignore matching monster species together. **Targets in this area** also provides the full zone list. All eligible species are allowed by default. Empty patrols and failed routes retry while enabled. Optional chocobo, food, combat FATEs in Automatic mode, auto respawn, and a stop level. The catalog covers 44 ARR spawn groups up to level 49.
